...
class Utils {
    ...
    private static boolean isSdcard(String path) {
    ...
    private static File getStorage(Context context) {
        ...
        try {
            ...
            Log.d(TAG, "Found " + volumes.length + " storage volumes");

            for (Object volume: volumes) {
                String path = (String) getPath.invoke(volume);
                boolean removable = (Boolean) isRemovable.invoke(volume);
                String state = (String) getState.invoke(storageManager, path);
                Log.d(TAG, "Volume: path=" + path + ", removable=" + removable + ", state=" + state);

                //if (removable && !isSdcard(path)
                if (removable &&
                    (Environment.MEDIA_MOUNTED.equals(state) ||
                        Environment.MEDIA_MOUNTED_READ_ONLY.equals(state))) {

                    File dir = new File(path);
                    if (dir.exists() && dir.isDirectory() && containsVideoFile(dir)) {
                        Log.d(TAG, "Selected storage directory: " + path);
                        return dir;
                    }
                }
            }
            ...
            if (files != null) {
                ...
                for (File f : files) {
                    if (f.isDirectory() && f.canRead()
                            && !f.getAbsolutePath().equals(primaryPath)
                            && !isSdcard(f.getAbsolutePath())
                            && containsVideoFile(f)) {
    ...
    public static TreeMap < String, Uri > getVideos(Context context) {
        TreeMap < String, Uri > videoFiles = new TreeMap < > ();
        File storageDir = getStorage(context);
        if (storageDir != null && storageDir.exists() && storageDir.isDirectory()) {
            File[] files = storageDir.listFiles();
            Log.d(TAG, "Scanning storage: " + storageDir.getAbsolutePath() + ", total files: " + (files != null ? files.length : 0));
            if (files != null) {
                for (File f: files) {
                    if (f.isFile() && isVideoFile(f)) {
                        ...
                        videoFiles.put(name, uri);
                        Log.d(TAG, "Found video file: " + name + " -> " + uri);
                    }
                }
            }
        }
    ...
    private static boolean containsVideoFile(File dir) {
